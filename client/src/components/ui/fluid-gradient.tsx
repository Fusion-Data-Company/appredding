...
const defaultConfig: Required<FluidConfig> = {
  brushSize: 25.0,
  brushStrength: 0.5,
...
function FluidSimulation({ config }: { config: Required<FluidConfig> }) {
...
export function FluidGradient({ className = '', config: userConfig = {} }: FluidGradientProps) {
  ...
  return (
    ...
    >
      ...
      <FluidSimulation key={simKey} config={config} />
    </Canvas>
  );
}
